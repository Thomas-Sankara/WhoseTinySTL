...
namespace WhoseTinySTL{
    ...
    public:
        ...
        // operator==和operator!=实例化出来的友元函数认为是相同的函数了，也就是发生了函数重定义错误。
        // 编译器对着模板报什么non-function的错误。大佬做这种推测的理由之一是：把vector内的两个比较运算符注释掉
        // ——当然，由于友元函数的实现就是调用vector的比较运算符，也得随便改改内容，不调用vector内的比较运算符就行——
        // 然后使用《C++ Primer》589页推荐的写法，也就是上面我注释掉的那两个行，最后是能通过编译并正常运行，
        // 并不会报错。这就是友元函数模板实例化与成员函数冲突的一个佐证。事实也是这样，我意外地找到了原因：
        // https://blog.csdn.net/taodm/article/details/15759?spm=1001.2014.3001.5506
        // "你能显式申明友元函数是一个模板（的实例）。然而，如果你没有，并且如果它不匹配于已经被申明的函数模板，
        // 那么编译器将假设它是一个普通的非模板函数。"
    };// end of class vector
}
